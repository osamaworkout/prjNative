...
function populateLegend(legendEl, data) {
    legendEl.innerHTML = data.map((value, i) => `
        <li>
            ...
            <span style="margin-right: 8px; color: #666;">${formatNumber(value)}</span>
        </li>
    `).join('');
}
...
function populateTable(tableEl, data) {
    ...
    tbody.innerHTML = data.map((value, i) => `
        <tr>
            ...
            <td>${labels[i]}</td>
            <td><span style="background-color: ${colors[i]}; width: 16px; height: 16px; display: inline-block; border-radius: 4px;"></span></td>
        </tr>
    `).join('');
}
...
function updateCardStats(data) {
    // Update Users Info
    if (data.users) {
        document.getElementById('totalUsers').textContent = data.users.total || '125';
        document.getElementById('activeUsers').textContent = data.users.active || '98';
    }
    
    // Update Cars Info
    if (data.cars) {
        document.getElementById('totalCars').textContent = data.cars.total || '1283';
        document.getElementById('activeCars').textContent = data.cars.active || '1256';
    }
    
    // Update Drivers Info
    if (data.drivers) {
        document.getElementById('totalDrivers').textContent = data.drivers.total || '156';
        document.getElementById('activeDrivers').textContent = data.drivers.active || '134';
    }
} 